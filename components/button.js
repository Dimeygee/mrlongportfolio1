...
export const Button = ({ color, bgcolor, text, px, py, opacity, fontWeight, ifDark, ifSectionStyle, resume, mail, seemore }) => {
    ...
    style={darkStyle} 
    className="transition duration-[0.5s] cursor-pointer border-opacity-10 rounded-[28px] bg-[#F7FAFC] hover:bg-[#1A202C] hover:text-[#F7FAFC] text-[#1A202C] text-center text-base font-outfit tracking-[-0.36px] border-2 border-mrlongblue dark:border-transparent`"  target="__blank">{text}</a>) : (
    <a style={style}  className="transition cursor-pointer duration-[0.5s] border-opacity-10 bg-[#2F80ED] hover:bg-[#F7FAFC] text-[#fff] hover:text-[#2F80ED] rounded-[28px] text-center text-base font-outfit tracking-[-0.36px] border-2 border-mrlongblue dark:border-transparent`" target="__blank">{text}</a>
    )
